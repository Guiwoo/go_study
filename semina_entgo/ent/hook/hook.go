...
// The TesterFunc type is an adapter to allow the use of ordinary
// function as Tester mutator.
type TesterFunc func(context.Context, *ent.TesterMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TesterFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TesterMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TesterMutation", m)
}

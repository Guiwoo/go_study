...
func (cq *CardQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Card, init func(*Card), assign func(*Card, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(card.OwnerColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.card_owner
		if fk == nil {
			return fmt.Errorf(`foreign-key "card_owner" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "card_owner" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
